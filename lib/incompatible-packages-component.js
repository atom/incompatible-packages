...
export default class IncompatiblePackagesComponent {
  ...
  renderIncompatiblePackage (pack) {
    ...
    return (
      <div className={'incompatible-package'}>
        ...
        {this.renderRebuildButtons(pack, rebuildStatus)}
        {
          rebuildStatus
  ...
  renderRebuildButtons(pack, rebuildStatus) {
    if (rebuildStatus === REBUILD_FAILED) {
      return (
        <div className='btn-group'>
          <button ref='uninstallButton' className='btn icon icon-trashcan'>Uninstall</button>
        </div>
      )
    }
  }
